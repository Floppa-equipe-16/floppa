...
public class SellerResource {
    ...
    public Response createSeller(@Context UriInfo uriInfo, SellerRequest sellerRequest) {
        verificationOfSellerNullParameter(sellerRequest);
        Seller seller = new Seller(sellerRequest.name, sellerRequest.birthdate, sellerRequest.email, sellerRequest.phoneNumber, sellerRequest.bio);
        sellers.add(seller);
        return Response.status(Response.Status.CREATED).header("Location", uriInfo.getAbsolutePath() + "/" + seller.getId()).build();
    }


    private SellerResponse getSellerResponseById(String sellerId) {
        Seller foundSeller = sellers.stream().filter(seller ->
                seller.getId().equals(sellerId)).findFirst().orElseThrow(() ->
                new NotFoundException(String.format("Seller with id '%s' not found", sellerId)));

        return new SellerResponse(foundSeller.getId(), foundSeller.getCreatedAt(), foundSeller.getName(),
                foundSeller.getBirthdate(), foundSeller.getEmail(), foundSeller.getPhoneNumber(), foundSeller.getBio());
    }


    protected void verificationOfSellerNullParameter(SellerRequest sellerRequest){
        if (sellerRequest.name == null)
            throw new SellerException(new Suberror("MISSING_PARAMETER","Missing name value"));
        if (sellerRequest.email == null)
        ...
        if (sellerRequest.phoneNumber == null)
